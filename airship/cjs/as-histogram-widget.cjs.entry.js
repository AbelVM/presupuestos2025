...
const index = require('./index-74a4f8ef.js');
const index$1 = require('./index-a0ac52a6.js');
const index$2 = require('./index-f86f0d00.js');
const constants = require('./constants-e65c6c4c.js');

function noevent() {
  index.event.preventDefault();
  index.event.stopImmediatePropagation();
}

function dragDisable(view) {
  var root = view.document.documentElement,
      selection = index.select(view).on("dragstart.drag", noevent, true);
...
function yesdrag(view, noclick) {
  var root = view.document.documentElement,
      selection = index.select(view).on("dragstart.drag", null);
  if (noclick) {
...
function toucher(identifier) {
  return function(target) {
    return index.touch(target, index.event.touches, identifier);
  };
}
...
function defaultFilter() {
  return !index.event.ctrlKey && !index.event.button;
}
...
function brush(dim) {
  var extent = defaultExtent,
      ...
      keys = true,
      listeners = index$1.dispatch("start", "brush", "end"),
      handleSize = 6,
      touchending;
      ...
      group
          ...
          .tween("brush", function() {
            var that = this,
                ...
                i = index.interpolate(selection0, selection1);
  ...
  Emitter.prototype = {
    ...
    emit: function(type) {
      index.customEvent(new BrushEvent(brush, type, dim.output(this.state.selection)), listeners.apply, listeners, [type, this.that, this.args]);
    }
  };

  function started() {
    if (touchending && !index.event.touches) return;
    if (!filter.apply(this, arguments)) return;

    var that = this,
        type = index.event.target.__data__.type,
        mode = (keys && index.event.metaKey ? type = "overlay" : type) === "selection" ? MODE_DRAG : (keys && index.event.altKey ? MODE_CENTER : MODE_HANDLE),
        ...
        dy = 0,
        moving,
        shifting = signX && signY && keys && index.event.shiftKey,
        lockX,
        lockY,
        pointer = index.event.touches ? toucher(index.event.changedTouches[0].identifier) : index.mouse,
        point0 = pointer(that),
        point = point0,
    ...
    var group = index.select(that)
        .attr("pointer-events", "none");

    var overlay = group.selectAll(".overlay")
        .attr("cursor", cursors[type]);

    if (index.event.touches) {
      emit.moved = moved;
      emit.ended = ended;
    } else {
      var view = index.select(index.event.view)
    ...
    function ended() {
      nopropagation();
      if (index.event.touches) {
        if (index.event.touches.length) return;
        if (touchending) clearTimeout(touchending);
        touchending = setTimeout(function() { touchending = null; }, 500); // Ghost clicks are delayed!
      } else {
        yesdrag(index.event.view, moving);
...
function getYDomain(data) {
    return [Math.min(0, getLowerBounds(data)), index.max(data, (d) => d.value)];
}
function getLowerBounds(data) {
    return index.min(data, (d) => d.value);
}
...
const formatter = index.format('.2~s');
const decimalFormatter = index.format('.3~f');
...
function renderXAxis(container, domain, bins, X_PADDING, Y_PADDING, customFormatter = conditionalFormatter, axisOptions) {
    ...
    const xScale = index.linear()
        .domain([0, bins])
        .range([0, WIDTH]);
    const realScale = index.linear()
        .domain(domain)
        .range([0, bins]);
    let xAxis;
    if (axisOptions.values || axisOptions.format) {
        const altScale = index.linear()
            .domain(domain)
            .range([0, WIDTH]);
        xAxis = index$2.axisBottom(altScale)
            .tickValues(axisOptions.values || null)
            .tickFormat(axisOptions.format || customFormatter);
    }
    else {
        xAxis = index$2.axisBottom(xScale)
...
function generateYScale(container, domain, X_PADDING, Y_PADDING, axisOptions) {
    ...
    const yScale = index.linear()
        .domain(domain)
        .range([HEIGHT, 0]);
    const yAxis = index$2.axisLeft(yScale)
        .tickSize(-WIDTH)
        .ticks(ticks)
        .tickPadding(tickPadding)
...
function addTooltip(container, barsContainer, hasSelection, color, unselectedColor, formatter, setTooltip, className) {
    container.on('mousemove', () => {
        ...
        _forEachRect(barsContainer, clientX, clientY, className, async (data, node, bucketIndex, boundingBox) => {
            ...
            _color = styles.shadeOrBlend(-0.16, _color);
...
function _forEachRect(container, x, y, className, insideCallback, outsideCallback) {
    container.selectAll(`rect.${className}`)
        .each((data, i, nodes) => {
        const nodeSelection = index.select(nodes[i]);
        const node = nodes[i];
...
const HistogramWidget = class {
    constructor(hostRef) {
        ...
        this.selectionInput = core.createEvent(this, "selectionInput", 7);
        this.drawParametersChanged = core.createEvent(this, "drawParametersChanged", 7);
    }
    ...
    _onColorChanged(newColor) {
        const incomingColor = newColor || constants.DEFAULT_BAR_COLOR;
        this._color = this._toColor(incomingColor, constants.DEFAULT_BAR_COLOR_HEX);
    }
    _onSelectedColorChanged(newColor) {
        const incomingColor = newColor || constants.DEFAULT_BACKGROUND_BAR_COLOR;
        this._barBackgroundColor = this._toColor(incomingColor, constants.DEFAULT_BACKGROUND_BAR_COLOR_HEX);
    }
    /**
    ...
    componentDidLoad() {
        this._color = this._toColor(this.color, constants.DEFAULT_BAR_COLOR_HEX);
        this._barBackgroundColor = this._toColor(this.unselectedColor, constants.DEFAULT_BACKGROUND_BAR_COLOR_HEX);
    ...
    render() {
        if (this._isLoading()) {
            return (core.h("as-histogram-widget-placeholder", null, this._renderHeader()));
        }
        return [
            this._renderHeader(),
    ...
    _renderContent() {
        ...
        return content_fragment.contentFragment(this._isLoading(), this.error, this._isEmpty(), this.heading, this.errorDescription, this.noDataBodyMessage, core.h("div", { class: histogramClasses }, core.h("svg", { class: svgClasses, ref: (ref) => this.container = index.select(ref) }), this._renderLabels(), this._renderTooltip()));
    }
    ...
    _renderSelection() {
        ...
        return core.h("as-widget-selection", { selection: this.selectionFooter, clearText: this.clearText, showClear: !this.selectionEmpty, onClear: () => this.clearSelection() });
    }
    _renderGraph() {
    ...
    _onBrush() {
        ...
        const evt = index.event; // I can't cast this properly :(
    ...
    _showTooltip(barBoundingBox) {
        ...
        index.select(this.tooltipElement)
    ...
    _renderHeader() {
        ...
        return core.h("as-widget-header", { header: this.heading, subheader: this.description, "is-loading": this._isLoading(), "is-empty": this._isEmpty(), error: this.error, "no-data-message": this.noDataHeaderMessage });
    }
    _renderTooltip() {
        return (core.h("span", { ref: (ref) => this.tooltipElement = ref, role: 'tooltip', class: 'as-tooltip as-tooltip--top' }, this._parseTooltip(this.tooltip)));
    }
    ...
    _renderTooltipLine(value) {
        return core.h("div", null, value);
    }
    _renderLabels() {
        return [
            this.yLabel ? core.h("div", { class: 'y-label' }, this.yLabel) : '',
            this.xLabel ? core.h("div", { class: 'x-label' }, this.xLabel) : '',
        ];
    }
    /**
    ...
    formatter(data) {
        ...
        tooltip.push(`${readableNumber.readableNumber(data.value).trim()}`);
        return tooltip;
    }
    _xFormatter(value) {
        if (this.axisFormatter) {
...
exports.as_histogram_widget = HistogramWidget;
