...
const CategoryWidget = class {
    ...
    _renderContent() {
        return content_fragment.contentFragment(false, this.error, this._isEmpty(), this.heading, this.errorDescription, this.noDataBodyMessage, this._renderCategoryList());
    }
    _renderCategoryList() {
        const cssClasses = {
    ...
    _getBarColor(color, options = {}) {
        ...
        if (options.isSelected) {
            return styles.shadeOrBlend(-0.16, color);
        }
        return color;
    }
    _parseCategories() {
