...
import { r as rgbBasis, c as cubehelixLong, a as cubehelix, b as rgb, l as linear, s as select, e as event } from './index-ba7acc67.js';
import './index-41aa841a.js';
import './index-87eae0eb.js';
import { y as yAxisService } from './y-axis.service-f48f1025.js';
...
function create(keys, metadata) {
    ...
    var scale = linear()
        ...
        .range([0, 1]);
    keys.forEach(function (key) { return map[key] = _getColor(key, keys, metadata, scale); });
    return map;
}
...
var colorMapFactory = { create: create };
/**
...
function getDomain(data) {
    return data.reduce(function (domain, currentValue) {
        var positiveAcum = 0;
        var negativeAcum = 0;
        for (var _i = 0, _a = Object.keys(currentValue.values); _i < _a.length; _i++) {
            var key = _a[_i];
            var value = currentValue.values[key];
            if (value >= 0) {
                positiveAcum += value;
            }
            if (value < 0) {
...
function rawDataToStackBarData(data, scale, colorMap, width, margin) {
    var origin = _getZeroAxis(scale);
    var xOffset = margin / 2;
    var result = [];
    for (var _i = 0, data_1 = data; _i < data_1.length; _i++) {
        var rawColumn = data_1[_i];
...
function getKeys(data) {
    ...
    for (var _i = 0, data_2 = data; _i < data_2.length; _i++) {
        var rawColumn = data_2[_i];
        Object.keys(rawColumn.values).forEach(function (key) {
            keys.add(key);
        });
    }
    var result = [];
    keys.forEach(function (key) { return result.push(key); });
    return result;
}
...
function _generateColumn(data, scale, colorMap, width, origin, x) {
    var column = [];
    var yOffset = origin;
    var _a = _split(data), positives = _a[0], negatives = _a[1];
    negatives.forEach(function (key) {
...
function _addRectangleAndGetHeight(column, data, key, scale, colorMap, x, w, yOffset, positive) {
    ...
    var h = _getRectSize(v, scale);
    var y = positive ? yOffset - h : yOffset;
    var c = colorMap[key];
    column.push({ c: c, h: h, v: v, w: w, x: x, y: y, });
    return h;
}
/**
...
function _split(data) {
    var negatives = [];
    var positives = [];
    for (var _i = 0, _a = Object.keys(data.values); _i < _a.length; _i++) {
        var key = _a[_i];
...
var dataService = { getDomain: getDomain, rawDataToStackBarData: rawDataToStackBarData, getKeys: getKeys, createLegendData: createLegendData };
var DURATION = 500;
/**
 * Draw the stacked bar char in the given svg element.
 */
function drawColumns(svgElement, data, mousemove, mouseleave) {
    var plot = _createPlot(svgElement);
    var columns = plot.selectAll('.column');
    // Remove columns
    columns
        .data(data)
        .exit()
...
var StackedBarWidget = /** @class */ (function () {
    function class_1(hostRef) {
        ...
        this.mouseOver = function (data) {
            ...
            _this.tooltip.style.left = event$1.clientX + "px";
            _this.tooltip.style.top = event$1.clientY + "px";
            _this.tooltip.innerText = "" + _this.formatFn(data.v);
        };
        /**
    ...
    class_1.prototype._renderContent = function () {
        ...
        return contentFragment(this.isLoading, this.error, this._isEmpty(), this.heading, this.errorDescription, this.noDataBodyMessage, [
            h("svg", { class: 'figure', ref: function (ref) { return _this.container = ref; } }),
            this._renderLegend(),
            h("span", { ref: function (ref) { return _this.tooltip = ref; }, role: 'tooltip', class: 'as-tooltip as-tooltip--top' }, " TOOLTIP")
        ]);
    };
    class_1.prototype._drawFigure = function () {
        ...
        requestAnimationFrame(function () {
            var yAxis = _this._drawYAxis();
            _this._drawColumns(yAxis);
        });
    };
    class_1.prototype._drawColumns = function (yAxisElement) {
        ...
        var Y_AXIS_LABEL_WIDTH = 25; // We draw on the right of the yAxis labels
        var columnMargin = 4;
        var WIDTH = yAxisElement.getBoundingClientRect().width - Y_AXIS_LABEL_WIDTH - columnMargin;
        var COLUMN_WIDTH = (WIDTH / this.data.length) - columnMargin;
        if (COLUMN_WIDTH < BAR_WIDTH_THRESHOLD) {
            WIDTH += columnMargin;
            COLUMN_WIDTH += columnMargin;
            columnMargin = 0;
        }
        var data = dataService.rawDataToStackBarData(this.data, this.scale, this.colorMap, COLUMN_WIDTH, columnMargin);
    ...
    class_1.prototype._renderLegend = function () {
        if (this.showLegend && this.colorMap) {
            var legendData = dataService.createLegendData(this.metadata, this.colorMap);
